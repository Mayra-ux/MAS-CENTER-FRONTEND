...
import {newBlogObsoleteRules, updateBlogObsoleteRules} from '../../actions/blogObsoleteRules';
...
export const EditModalObsolete= () => {
    const dispatch = useDispatch();
    const date= new Date();
    const year= date.getFullYear();
    

    let activeDoc = useSelector(state => state.obsolete.activeDoc);
    ...
    const handleSubmitForm = (e) => {
        ...
        if (!date) {
                Swal.fire('Error', "Please enter the date of Obsolete Rules image", 'error');
                return;
        }
        ...
        if(activeDoc ===null){
            dispatch(newBlogObsoleteRules(date, image));
