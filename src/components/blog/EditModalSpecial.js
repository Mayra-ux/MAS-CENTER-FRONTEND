...
import { updateBlogObsoleteRules } from '../../actions/blogObsoleteRules';
import { newBlogSpecialRules, updateBlogSpecialRules } from '../../actions/blogSpecialRules';
...
export const EditModal = () => {
    ...
    let activeDoc = useSelector(state => state.special.activeDoc);
    ...
    const closeModal = () =>{
      ...
      const handleSubmitForm = (e) => {
        ...
        if (!date) {
                Swal.fire('Error', "Please enter the date of erizo image", 'error');
                return;
        }
        ...
        if(activeDoc ===null){
            dispatch(newBlogSpecialRules(date, image));
            
        }else{
            dispatch(updateBlogObsoleteRules(activeDoc.date, image, activeDoc.id));
